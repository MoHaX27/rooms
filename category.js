window.roomCategories = {
	0: { //  Затычка что бы не крашило
            basePrice: 0,
            extraPrice: 0,
            baseOverPrice: 0,
            extraOverPrice: 0,
            maxBaseGuests: 0,
	    maxExtraGuests: 0,
            printName: "Null",
	    checkName: "Null"
        },
	////////////////////////// СТРЕЖЕНЬ /////////////////////////
        "184076863": { //  Стандарт 2х местный
            basePrice: 2080, // Стоимость основного места
            extraPrice: 1200, // Стоимость дополнительного места
            baseOverPrice: 2900, // Стоимость основного места ВЫСОКИЙ СЕЗОН
            extraOverPrice: 1490, // Стоимость дополнительного места ВЫСОКИЙ СЕЗОН
            maxBaseGuests: 2, // Максимальное количество основных мест
	    maxExtraGuests: 0, // Максимальное количество доп мест
            printName: "Стандарт", // Категоря номера для расшифровки
	    checkName: "Стандарт 2х местный" // Категоря номера для счета
        },
		"1839888264": { //  Одноместное размещение в Стандарт
            basePrice: 4160,
            extraPrice: 0,
            baseOverPrice: 5800,
            extraOverPrice: 0,
            maxBaseGuests: 1,
	    maxExtraGuests: 0,
            printName: "Стандарт",
	    checkName: "Одноместное размещение в 2х мест"
        },
        "3628094238": { //  Стандарт (2+1) 2х местный
            basePrice: 2080,
            extraPrice: 1200,
            baseOverPrice: 2900,
            extraOverPrice: 1490,
            maxBaseGuests: 2,
	    maxExtraGuests: 1,
            printName: "Стандарт",
	    checkName: "Стандарт 2х местный"
        },
        "3682376564": { //  Стандарт 3х местный
            basePrice: 1880,
            extraPrice: 1200,
            baseOverPrice: 2480,
            extraOverPrice: 1490,
            maxBaseGuests: 3,
	    maxExtraGuests: 0,
            printName: "Стандарт",
	    checkName: "Стандарт 3х местный"
        },
        "1889936227": { //  Двухместное размещение в 3х мест
            basePrice: 2460,
            extraPrice: 1200,
            baseOverPrice: 3050,
            extraOverPrice: 1490,
            maxBaseGuests: 2,
	    maxExtraGuests: 0,
            printName: "Стандарт в 3х",
	    checkName: "Двухместное размещение в 3х мест"
        },
        "372022831": { //  Комфорт 2 местный 2х комнатный
            basePrice: 3200,
            extraPrice: 1340,
            baseOverPrice: 3530,
            extraOverPrice: 1610,
            maxBaseGuests: 2,
	    maxExtraGuests: 0,
            printName: "Комфорт",
	    checkName: "Комфорт 2х местный 2х комнатный"
        },
        "3121715328": { //  Комфорт (2+1)
            basePrice: 3200,
            extraPrice: 1340,
            baseOverPrice: 3530,
            extraOverPrice: 1610,
            maxBaseGuests: 2,
	    maxExtraGuests: 1,
            printName: "Комфорт",
	    checkName: "Комфорт 2х местный 2х комнатный"
        },
        "2844400129": { //  Комфорт (2+2)
            basePrice: 3200,
            extraPrice: 1340,
            baseOverPrice: 3530,
            extraOverPrice: 1610,
            maxBaseGuests: 2,
	    maxExtraGuests: 2,
            printName: "Комфорт",
	    checkName: "Комфорт 2х местный 2х комнатный"
        },
        "1334863395": { //  Дуплекс 2х местный 2х комнатный
            basePrice: 2520,
            extraPrice: 1200,
            baseOverPrice: 3310,
            extraOverPrice: 1490,
            maxBaseGuests: 2,
	    maxExtraGuests: 0,
            printName: "Дуплекс",
	    checkName: "Дуплекс 2х местный 2х комнатный"
        },
        "1204873745": { //  Дуплекс (2+1)
            basePrice: 2520,
            extraPrice: 1200,
            baseOverPrice: 3310,
            extraOverPrice: 1490,
            maxBaseGuests: 2,
	    maxExtraGuests: 1,
            printName: "Дуплекс",
	    checkName: "Дуплекс 2х местный 2х комнатный"
        },
        "4109240830": { //  Улучшенный Стандарт 2х местный
            basePrice: 2400,
            extraPrice: 1340,
            baseOverPrice: 3110,
            extraOverPrice: 1610,
            maxBaseGuests: 2,
	    maxExtraGuests: 0,
            printName: "Улучш Стандарт",
	    checkName: "Улучшенный Стандарт 2х местный"
        },
        "1463994523": { //  Улучшенный Стандарт (2+1) 2х местный
            basePrice: 2400,
            extraPrice: 1340,
            baseOverPrice: 3110,
            extraOverPrice: 1610,
            maxBaseGuests: 2,
	    maxExtraGuests: 1,
            printName: "Улучш Стандарт",
	    checkName: "Улучшенный Стандарт 2х местный"
        },
        "2570918383": { //  Семейный 4х местный 2х комнатный
            basePrice: 1840,
            extraPrice: 1200,
            baseOverPrice: 2350,
            extraOverPrice: 1490,
            maxBaseGuests: 4,
	    maxExtraGuests: 0,
            printName: "Семейный",
	    checkName: "Семейный 4х местный 2х комнатный"
        },
        "2729833985": { //  Семейный (4+1)
            basePrice: 1840,
            extraPrice: 1200,
            baseOverPrice: 2350,
            extraOverPrice: 1490,
            maxBaseGuests: 4,
	    maxExtraGuests: 1,
            printName: "Семейный",
	    checkName: "Семейный 4х местный 2х комнатный"
        },
	////////////////////////// АЛЫЕ ПАРУСА /////////////////////////
	"1428382152": { //  Стандарт 2-х местный (место в номере) 
            basePrice: 2270,
            extraPrice: 1010,
            baseOverPrice: 2270,
            extraOverPrice: 1010,
            maxBaseGuests: 1,
	    maxExtraGuests: 1,
            printName: "2х Стандарт",
	    checkName: "Стандарт 2х местный"
        },
	"620115699": { //  1-но местный стандарт 
            basePrice: 3170,
            extraPrice: 1010,
            baseOverPrice: 3170,
            extraOverPrice: 1010,
            maxBaseGuests: 1,
	    maxExtraGuests: 0,
            printName: "1х Стандарт",
	    checkName: "1но местный стандарт"
        },
	"2038853658": { //  2-х местный 2-х комнатный Комфорт 
            basePrice: 2930,
            extraPrice: 1640,
            baseOverPrice: 2930,
            extraOverPrice: 1640,
            maxBaseGuests: 2,
	    maxExtraGuests: 0,
            printName: "Комфорт",
	    checkName: "2х местный 2х комнатный Комфорт"
        },
	"3870794209": { //  2-х местный 2-х комнатный Комфорт(2+1) 
            basePrice: 2930,
            extraPrice: 1640,
            baseOverPrice: 2930,
            extraOverPrice: 1640,
            maxBaseGuests: 2,
	    maxExtraGuests: 1,
            printName: "Комфорт",
	    checkName: "2х местный 2х комнатный Комфорт"
        },
	"62943580": { //  2-х местный 2-х комнатный Комфорт (2+2)
            basePrice: 2930,
            extraPrice: 1640,
            baseOverPrice: 2930,
            extraOverPrice: 1640,
            maxBaseGuests: 2,
	    maxExtraGuests: 2,
            printName: "Комфорт",
	    checkName: "2х местный 2х комнатный Комфорт"
        },
	"1505128695": { //  Люкс-Cтудия
            basePrice: 3020,
            extraPrice: 1640,
            baseOverPrice: 3020,
            extraOverPrice: 1640,
            maxBaseGuests: 2,
	    maxExtraGuests: 0,
            printName: "Люкс Студия",
	    checkName: "Люкс Студия"
        },
	"627656902": { //  Люкс-Студия 2-х местн. размещ. (2+1)
            basePrice: 3020,
            extraPrice: 1640,
            baseOverPrice: 3020,
            extraOverPrice: 1640,
            maxBaseGuests: 2,
	    maxExtraGuests: 1,
            printName: "Люкс Студия",
	    checkName: "Люкс Студия"
        },
	"2462037087": { //  Люкс-Студия 1-местн. размещ.
            basePrice: 5430,
            extraPrice: 1640,
            baseOverPrice: 5430,
            extraOverPrice: 1640,
            maxBaseGuests: 1,
	    maxExtraGuests: 0,
            printName: "1но мест Люкс Студия",
	    checkName: "Люкс Студия"
        },
	"3050570759": { //  Апт. 4-х местн. размещ. (4+1) 
            basePrice: 3140,
            extraPrice: 1710,
            baseOverPrice: 3140,
            extraOverPrice: 1710,
            maxBaseGuests: 4,
	    maxExtraGuests: 2,
            printName: "4х Аппартаменты",
	    checkName: "Аппартаменты"
        },
	"2120161402": { //  Аппартаменты 
            basePrice: 3140,
            extraPrice: 1710,
            baseOverPrice: 3140,
            extraOverPrice: 1710,
            maxBaseGuests: 4,
	    maxExtraGuests: 0,
            printName: "4х Аппартаменты",
	    checkName: "Аппартаменты"
        },
	"4130272292": { //  Апт. 3-местн. размещ. 
            basePrice: 3990,
            extraPrice: 1710,
            baseOverPrice: 3990,
            extraOverPrice: 1710,
            maxBaseGuests: 3,
	    maxExtraGuests: 0,
            printName: "3х Аппартаменты",
	    checkName: "Аппартаменты"
        },
	"466705239": { //  Апт. 2-местн. размещ. 
            basePrice: 5670,
            extraPrice: 1710,
            baseOverPrice: 5670,
            extraOverPrice: 1710,
            maxBaseGuests: 2,
	    maxExtraGuests: 0,
            printName: "2х Аппартаменты",
	    checkName: "Аппартаменты"
        },
	"2921127762": { //  Апт. 1-местн. размещ. 
            basePrice: 10720,
            extraPrice: 1710,
            baseOverPrice: 10720,
            extraOverPrice: 1710,
            maxBaseGuests: 1,
	    maxExtraGuests: 0,
            printName: "1х Аппартаменты",
	    checkName: "Аппартаменты"
        },
	"1746652214": { //  Стандарт 2х местный (место в ном)-Корпус 1
            basePrice: 2930,
            extraPrice: 1500,
            baseOverPrice: 2930,
            extraOverPrice: 1500,
            maxBaseGuests: 1,
	    maxExtraGuests: 1,
            printName: "2х Стандарт (Корпус 1)",
	    checkName: "Стандарт 2х местный (Корпус 1)"
        },
	"1427329284": { //  Стандарт 2х местный (место в ном)-Корпус 1 (1+1) 
            basePrice: 2930,
            extraPrice: 1500,
            baseOverPrice: 2930,
            extraOverPrice: 1500,
            maxBaseGuests: 1,
	    maxExtraGuests: 1,
            printName: "2х Стандарт (Корпус 1)",
	    checkName: "Стандарт 2х местный (Корпус 1)"
        },
	"242212083": { //  1но местный - Корпус 1
            basePrice: 3890,
            extraPrice: 1500,
            baseOverPrice: 3890,
            extraOverPrice: 1500,
            maxBaseGuests: 1,
	    maxExtraGuests: 1,
            printName: "1но Стандарт (Корпус 1)",
	    checkName: "Стандарт 1но местный (Корпус 1)"
        },
	"1481063373": { //  Улучшенный Стандарт 2х мест 
            basePrice: 3090,
            extraPrice: 1500,
            baseOverPrice: 3090,
            extraOverPrice: 1500,
            maxBaseGuests: 1,
	    maxExtraGuests: 1,
            printName: "Улучш Стандарт 2х мест (К1)",
	    checkName: "Улучшенный Стандарт 2х местный (К1)"
        },
	////////////////////////// ГОЛУБАЯ ГАВАНЬ /////////////////////////
	"3140483411": { //  1-но местный Стандарт 
            basePrice: 1730,
            extraPrice: 930,
            baseOverPrice: 1970,
            extraOverPrice: 1010,
            maxBaseGuests: 1,
	    maxExtraGuests: 0,
            printName: "Стандарт",
	    checkName: "1но местный Стандарт"
        },
	"2303439199": { //  2х местный Стандарт 
            basePrice: 1490,
            extraPrice: 930,
            baseOverPrice: 1820,
            extraOverPrice: 1010,
            maxBaseGuests: 2,
	    maxExtraGuests: 0,
            printName: "Стандарт",
	    checkName: "2х местный Стандарт"
        },
	"3675283048": { //  2х местный Стандарт (2+1) 
            basePrice: 1490,
            extraPrice: 930,
            baseOverPrice: 1820,
            extraOverPrice: 1010,
            maxBaseGuests: 2,
	    maxExtraGuests: 1,
            printName: "Стандарт",
	    checkName: "2х местный Стандарт"
        },
	"3576145298": { //  3х местный Стандарт
            basePrice: 1260,
            extraPrice: 930,
            baseOverPrice: 1710,
            extraOverPrice: 1010,
            maxBaseGuests: 3,
	    maxExtraGuests: 0,
            printName: "Стандарт",
	    checkName: "3х местный Стандарт"
        },
	"370570838": { //  3х местный Стандарт (3+1)
            basePrice: 1260,
            extraPrice: 930,
            baseOverPrice: 1710,
            extraOverPrice: 1010,
            maxBaseGuests: 3,
	    maxExtraGuests: 1,
            printName: "Стандарт",
	    checkName: "3х местный Стандарт"
        },
	"1831337534": { //  2х местный 2х комнатный Комфорт 
            basePrice: 2160,
            extraPrice: 1000,
            baseOverPrice: 2490,
            extraOverPrice: 1100,
            maxBaseGuests: 2,
	    maxExtraGuests: 0,
            printName: "Комфорт",
	    checkName: "2х местный 2х комнатный Комфорт"
        },
	"2889645082": { //  2х местный 2х комнат Комфорт (2+1) 
            basePrice: 2160,
            extraPrice: 1000,
            baseOverPrice: 2490,
            extraOverPrice: 1100,
            maxBaseGuests: 2,
	    maxExtraGuests: 1,
            printName: "Комфорт",
	    checkName: "2х местный 2х комнатный Комфорт"
        },
	"3477010176": { //  2х местный 2х комнатный Комфорт (2+2) 
            basePrice: 2160,
            extraPrice: 1000,
            baseOverPrice: 2490,
            extraOverPrice: 1100,
            maxBaseGuests: 2,
	    maxExtraGuests: 2,
            printName: "Комфорт",
	    checkName: "2х местный 2х комнатный Комфорт"
        },
	"2782405903": { //  2х мест 2х комнат Комфорт (с кухней) 
            basePrice: 2500,
            extraPrice: 980,
            baseOverPrice: 2730,
            extraOverPrice: 1100,
            maxBaseGuests: 2,
	    maxExtraGuests: 0,
            printName: "Комфорт (с кухней)",
	    checkName: "2х мест 2х комнат Комфорт (с кухней)"
        },
	"2176035421": { //  2х мест 2х комнат Комфорт (с кухней (2+1) 
            basePrice: 2500,
            extraPrice: 980,
            baseOverPrice: 2730,
            extraOverPrice: 1100,
            maxBaseGuests: 2,
	    maxExtraGuests: 1,
            printName: "Комфорт (с кухней)",
	    checkName: "2х мест 2х комнат Комфорт (с кухней)"
        },
	"3201505495": { //  2х местный 2х комнатный Комфорт (с кухней) (2+2)
            basePrice: 2500,
            extraPrice: 980,
            baseOverPrice: 2730,
            extraOverPrice: 1100,
            maxBaseGuests: 2,
	    maxExtraGuests: 2,
            printName: "Комфорт (с кухней)",
	    checkName: "2х мест 2х комнат Комфорт (с кухней)"
        },
	"1653107628": { //  4х местный 2х комнатный Семейный (с кухней)
            basePrice: 1800,
            extraPrice: 930,
            baseOverPrice: 2210,
            extraOverPrice: 1010,
            maxBaseGuests: 4,
	    maxExtraGuests: 0,
            printName: "4х Комфорт (с кухней)",
	    checkName: "4х местный 2х комнатный Семейный (с кухней)"
        },
	"867276793": { //  4х местный 2х комнатный Семейный (с кухней) (4+1)
            basePrice: 1800,
            extraPrice: 930,
            baseOverPrice: 2210,
            extraOverPrice: 1010,
            maxBaseGuests: 4,
	    maxExtraGuests: 1,
            printName: "4х Комфорт (с кухней)",
	    checkName: "4х местный 2х комнатный Семейный (с кухней)"
        },
	"3644462357": { //  2х местный Эконом
            basePrice: 1000,
            extraPrice: 0,
            baseOverPrice: 1090,
            extraOverPrice: 0,
            maxBaseGuests: 2,
	    maxExtraGuests: 0,
            printName: "2х Эконом",
	    checkName: "2х местный Эконом"
        },
	"4234027792": { //  3х местный Эконом
            basePrice: 910,
            extraPrice: 0,
            baseOverPrice: 1000,
            extraOverPrice: 0,
            maxBaseGuests: 3,
	    maxExtraGuests: 0,
            printName: "3х Эконом",
	    checkName: "3х местный Эконом"
        },
    };

 window.baza = {
        "1535": {
            name: "АП",
            foodPrice: 1630,
            medicine: 1200,
	    	checkName: "с/п Алые паруса"
        },
        "1534": {
            name: "Стрежень",
            foodPrice: 1300,
	    	checkName: "б/о Стрежень"
        },
        "2756": {
            name: "ГГ",
            foodPrice: 1300,
	    	checkName: "б/о Голубая гавань"
        },
        "50423": {
            name: "Поршень",
            foodPrice: 1420,
	    	checkName: "б/о Поршень"
        }
    };

window.discount = {
        "null": { name: "" }, // Обоснование: именинник-20%
        "2033228569": { name: "Работник АВЗ" }, // Обоснование: работник группы АВТОВАЗ 20%
        "3597439000": { name: "Работник АВЗ" }, // Обоснование: Работник группы АВТОВАЗ- 20%   // АП
        "618700338": { name: "Именинник" }, // Обоснование: именинник-20%
        "2803113729": { name : "Пенсионер" }, // Обоснование: Пенсионер 20%
	"3033163572": { name : "Пенсионер" }, // Обоснование: Пенсионер - 20% // АП
	"4190677307": { name : "РБ" }, // Обоснование: Раннее бронирование 20% 
	"1977351661": { name : "РБ" }, // Обоснование: Раннее бронирование - 10%
	"3073493742": { name : "РБ" }, // Обоснование: Раннее бронирование - 20% 
    };
